using GfToolkit.Shared.Dtos.Models;
namespace GfToolkit.Shared.Dtos
{
    public class GameDataDto
    {
        public static GameDataDto Database { get; set; } = new();
        public List<ActorDto> Actors { get; set; } = new();
        // public static List<BehaviorDto> Behaviors { get; set; } = new();
        // public static List<SkillDto> Skills { get; set; } = new();
        // public static List<ItemDto> Items { get; set; } = new();
        // public static List<WeaponDto> Weapons { get; set; } = new();
        public List<BuffSetDto> BuffSets { get; set; } = new();
        // public static List<TraitDto> Traits { get; set; } = new();
    }
}